...
class RolloutTrainerMixin(RLHFTrainerMixin):
    ...
    def _colocate_multi_turn_infer(self, inputs: DataType, first_turn_rollout_outputs: List[RolloutOutput],
        ...
        while True:
            ...
            # First pass: handle stopped dialogues and collect pending (index, output) pairs
            pending_step_items = []  # (index, output) for non-stopped samples
            for stop, index, output in zip(should_stops, index_to_infer, outputs):
                ...
                if stop:
                    ...
                    pending_step_items.append((index, output))

            # Second pass: run step() for all pending samples in parallel.
            # Each step() is independent (operates on its own requests[index]), so
            # parallel execution is safe and eliminates serialized Reward API wait time.
            if pending_step_items:
                def _run_one_step(item):
                    idx, out = item
                    return idx, self.multi_turn_scheduler.step(
                        requests[idx], out.response.choices[0], current_turn)

                max_workers = len(pending_step_items)
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    step_results = dict(executor.map(_run_one_step, pending_step_items))

                for index, output in pending_step_items:
                    is_continuation = is_continuations[index]
